...
// In-memory config
let config: Config = configData as Config;
...
const generateRandomTriggers = (): string[] => {
...
const triggersInUse = (): Set<string> => {
  return new Set(Object.values(config.profiles).flatMap((p) => p.triggers));
};

const generateUniqueTriggers = (): string[] => {
...
// Get single profile
router.get("/:profile", (req: Request<{ profile: string }>, res: Response) => {
  const { profile } = req.params;
  const target = config.profiles[profile];
  if (!target) return res.status(404).send(createError(404, "Profile not found"));
  res.status(200).json(target);
});

// Create a new profile (with defaults but unique triggers)
router.post("/:profile", (req: Request<{ profile: string }>, res: Response) => {
  const { profile } = req.params;

  if (config.profiles[profile]) {
    return res.status(409).send(createError(409, "Profile already exists"));
  }
...
router.post(
  "/:profile/:param",
  (
    req: Request<{ profile: string; param: keyof Profile }, {}, { value: any }>,
    res: Response
  ) => {
    ...
    if (!target) return res.status(404).send(createError(404, "Profile not found"));
    if (profile === "default")
      return res.status(403).send(createError(403, "Cannot override default profile"));
    ...
    switch (param) {
      ...
      case "triggers":
        ...
        value = value.map((v: unknown) => String(v).replace(/^"(.*)"$/, "$1").trim());
        ...
        if (param === "triggers") {
          ...
          if (value.some((t:any) => used.has(t))) {
            return res
              .status(400)
              .send(createError(400, "Some triggers are already in use by another profile"));
          }
        }
      ...
      default:
        return res.status(400).send(createError(400, "Invalid parameter"));
    }
...
// Delete a profile
router.delete("/:profile", (req: Request<{ profile: string }>, res: Response) => {
  const { profile } = req.params;
  if (profile === "default")
    return res.status(403).send(createError(403, "Cannot delete default profile"));
  if (!config.profiles[profile])
    return res.status(404).send(createError(404, "Profile not found"));
...
router.post("/", (req: Request<{}, {}, Config>, res: Response) => {
  const newConfig = req.body;

  if (!newConfig.profiles || typeof newConfig.profiles !== "object") {
    return res.status(400).send(createError(400, "Invalid config format"));
  }
  ...
  if (triggerSet.size !== allTriggers.length) {
    return res
      .status(400)
      .send(createError(400, "Duplicate triggers found in provided config"));
  }
