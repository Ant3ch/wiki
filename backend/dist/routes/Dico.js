...
router.get("/:title", async (req, res) => {
    try {
        ...
        html = html.replace(/href="\/wiki\//g, 'href="https://fr.wikipedia.org/wiki/');
        html = html.replace(/href="\/w\//g, 'href="https://fr.wikipedia.org/w/');
        html = html.replace(/src="\/w\//g, 'src="https://fr.wikipedia.org/w/');
...
router.get("/main", async (req, res) => {
    try {
        const response = await fetch("https://fr.wiktionary.org/");
        let html = await response.text();
        html = html.replace(/href="\/wiki\//g, 'href="https://fr.wikipedia.org/wiki/');
        html = html.replace(/href="\/w\//g, 'href="https://fr.wikipedia.org/w/');
        html = html.replace(/src="\/w\//g, 'src="https://fr.wikipedia.org/w/');
...
router.get("/search/:query", async (req, res) => {
    try {
        ...
        const response = await fetch(`https://fr.wiktionary.org/w/api.php?action=query&list=search&srsearch=${encodeURIComponent(query)}&format=json`);
